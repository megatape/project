...
def start(message):
    connect = sqlite3.connect('bot.db') #название sql файл = bot
    cursor = connect.cursor()
# создаём таблицу, в которой будут пользователи
    cursor.execute("""CREATE TABLE IF NOT EXISTS info(
        id INTEGER
    )""")

    connect.commit()
# проверка на существование уже зарегестрированного пользователя
    ...
    # добавление id пользователей
        ...
        cursor.execute("INSERT INTO info VALUES(?);", bot_id)
        connect.commit()
    else:
    ...
    item3 = types.KeyboardButton('Раписание экзаменов')
